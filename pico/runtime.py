...
class Pico:
    ...
    # 把一段文本里出现的敏感环境变量值替换成统一的脱敏占位符 REDACTED_VALUE
    ...
    # 递归地对任意结构化数据做脱敏，防止里面的 secret 名称或 secret 值被泄露
    ...
    # 往运行时间线里写一条事件日志，事件里会自动脱敏敏感信息，防止泄露
    ...
    # 给工作区中每一个文件都计算指纹，然后以路径到指纹的映射形式返回整个工作区的快照。
    # 后续可以通过对比两次快照来判断哪些文件改了，甚至改了多少（如果指纹算法是增量的）
    ...
    # 基于当前任务执行状态，创建一个新的 checkpoint
    # 保存“当前做到哪里、关键文件是什么、下次怎么接着做”
    ...
    # 根据当前任务状态 task_state，推断一句“接下来应该做什么”的描述
    # 主要给 checkpoint 或恢复流程使用
    ...
    # 当某个工具执行得不顺利时，把“这次失败/部分成功的过程经验”记成一条 process note
    # 放进 memory 里，避免下一步马上重复踩坑
    ...
    # 判断一条 note 是否不适合进入 durable memory
    ...
    # 从本轮 final_answer 里提取“可以晋升为 durable memory 的笔记
